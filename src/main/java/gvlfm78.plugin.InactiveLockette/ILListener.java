...
public class ILListener implements Listener {
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if(line1.equalsIgnoreCase("[Private]") || //The first line of the sign contains [Private]
			...
			if(isBlackListed(ownerUUID)){ player.sendMessage(ILConfigHandler.mes("onPunch.noPermission")); return; }
			...
			if(ownerUUID != null && !ownerUUID.toString().isEmpty()){//Use UUIDs
				if(isOverlyInactive(ownerUUID)){
					...
					ownerStillActive(player,getInactivityDays(ownerUUID)); return;
				}
	...
	private void makeUserPay(Player p){
		if(!ILConfigHandler.config.getBoolean("useEconomy") || !(ILMain.econ.isEnabled() && ILMain.econ.hasAccount(p))) return;
	...
	private boolean isBlackListed(UUID ownerUUID){
		if(plugin.getConfig().getList("list") != null && !plugin.getConfig().getList("list").isEmpty()){
			...
			if(list.contains(ownerUUID) || list.contains(ownerUUID.toString())) return true;
		}
		return false;
	}
